...
struct screen_x11 {
    ...
    graph_app_t *graph_app;           // the application...
};

static XImage * create_ximage(screen_x11_t *screen_x11);
static void blit_screen(screen_x11_t *screen_x11);

static void init_app_screen_settings(const screen_x11_t *screen_x11);
...
screen_x11_t *init_screen_x11(int xsize, int ysize) {
    ...
    XStoreName(screen_x11->dpy, screen_x11->w, "Xlib graphics");
    ...
    screen_x11->ximg = create_ximage(screen_x11);
    screen_x11->buffer = (unsigned char *)(screen_x11->ximg->data);
//    screen_x11->buffer = malloc(xsize*ysize*screen_x11->bytespp);

    init_app_screen_settings(screen_x11);

    return screen_x11;
}

void init_app_screen_settings(const screen_x11_t *screen_x11) {
    screen_settings_t *screen_settings = malloc(sizeof(screen_settings_t));
    screen_settings->screen_buffer = screen_x11->buffer;
    screen_settings->red_mask = screen_x11->vis->red_mask;
    screen_settings->green_mask = screen_x11->vis->green_mask;
    screen_settings->blue_mask = screen_x11->vis->blue_mask;
    screen_settings->bytes_per_pixel = screen_x11->bytespp;
    screen_settings->bytes_per_rgb = screen_x11->depth / BITS_PER_BYTE;
    init_screen_settings(screen_x11->graph_app, screen_settings);
}
...
void run(screen_x11_t *screen_x11) {
    XEvent event;
    char ch;
    KeySym keysym;
...
//    while(!pipeline->done) {
    while(!done) {
        if(XCheckWindowEvent(screen_x11->dpy, screen_x11->w, KeyPressMask, &event)) {
            ...
            key_event(screen_x11->graph_app, ch);
            switch(ch) {
                case 'q': {
                    done = True;
                }
        ...
        update_event(screen_x11->graph_app);
        draw_event(screen_x11->graph_app);
        blit_screen(screen_x11);
...
static inline void blit_screen(screen_x11_t *screen_x11) {
