...
screen_x11_t *init_screen_x11(screen_settings_t *screen_settings) {
    ...
    int defaultDepth = DefaultDepth(screen_x11->display, 0);
    int count;
    ...
    switch (screen_x11->vis->class) {
        ...
        case TrueColor: {
            XVisualInfo vi;
            int result = XMatchVisualInfo(screen_x11->display, screen_num, screen_x11->depth, TrueColor, &vi);
            if (result) {
            ...
            break;
        case PseudoColor:
            printf("unsupported visual PseudoColor\n");
            exit(EXIT_FAILURE);
        case StaticColor:
            printf("unsupported visual StaticColor\n");
            exit(EXIT_FAILURE);
        case GrayScale:
            printf("unsupported visual GrayScale\n");
            exit(EXIT_FAILURE);
        case StaticGray:
            printf("unsupported visual StaticGray\n");
            exit(EXIT_FAILURE);
        case DirectColor:
            printf("unsupported visual DirectColor\n");
            exit(EXIT_FAILURE);
    }
...
//    screen_x11->buffer = malloc(xsize*ysize*screen_x11->bytespp); // not necessary with the shm
    ...
    screen_x11->show_rendering_info = screen_settings->show_rendering_info;
...
void run_event_loop(screen_x11_t *screen_x11,
    ...
    // move to dtor?
    XShmDetach(screen_x11->display, &screen_x11->shminfo);
...
void update(screen_x11_t *screen_x11, int64_t elapsed_time) {
    ...
    if(XCheckWindowEvent(screen_x11->display, screen_x11->window, KeyPressMask, &event)) {
